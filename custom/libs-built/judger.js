...
async function connect() {
  ...
  judgeNamespace.on('connect', socket => {
    ...
    socket.on('reportProgress', async (token, payload) => {
      ...
      // [v1.5.1] 检查是否被取消, 取消则不更新进度
      try {
        const judge_state_check = await JudgeState.findOne({ where: { task_id: progress.taskId } });
        if (judge_state_check && syzoj.cancelledJudgeIds && syzoj.cancelledJudgeIds.has(judge_state_check.id)) {
          return;
        }
      } catch (e) {}
    ...
    socket.on('reportResult', async (token, payload) => {
      ...
      // [v1.5.1] 检查取消标记: 如果该评测已被管理员取消, 抛弃 daemon 返回的结果
      if (judge_state && syzoj.cancelledJudgeIds && syzoj.cancelledJudgeIds.has(judge_state.id)) {
